...
def update_chapter():
    ...
    if story_id:
        ...
        if field == "summary":
            chapter=ChapterService.get_chapter_for_display(chapter_id)
            return jsonify({"success": "Record updated", "disp_summary": chapter["disp_summary"]}), 200
        else:
            return jsonify({"success": "Record updated"}), 200
...
def addChapterCharacter():
    ...
    return ({"success": True, "row_id": row_id}), 200
...
def print_chapter():
    ...
    return jsonify({"error": "Missing story or chapter id"}), 406
...
def summarise_chapter():
    ...
    if story_id and chapter_id:
        try:
            ...
            logging.debug(f"Error {e}")
            return jsonify({"error": "Summary generation failed"}), 406
    else:
        return jsonify({"error": "Missing story or chapter id"}), 406
